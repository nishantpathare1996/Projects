...
export class DivvyTripsChartComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    for (var i = 0; i < this.dates_of_week_days.length; i++) {
      ...
      this.getDivvyTripsCountsPerDay(
        daydata,
        new Date(this.dates_of_week_days[i])
      );
    }
  ...
  private initAxis() {
    ...
    this.y
      .domain([
        0,
        d3Array.max(this.divvyTripsCounts, (d) => Number(d.total_trips)),
      ])
      .nice();
  }
  ...
  private updateChartBars(data) {
    ...
    var bars = this.g.selectAll(".bar").remove().exit().data(data);
