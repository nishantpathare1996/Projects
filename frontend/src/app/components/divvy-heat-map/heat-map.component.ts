...
export class HeatMapComponent implements OnInit {
  ...
  checkForSingleDigitAndPrefixZeroIfNeeded(digitValue) {
    if (digitValue < 10) digitValue = "0" + digitValue;
  ...
  getDivvyStationsStatus(timeRangeSelection) {
    ...
    // Based on the time-range selected we specify how many animation frames to display on th heatmap
    ...
    if (this.timeRangeSelected == this.PAST_HOUR) {
      ...
      startTimeForDataCollection = new Date(
        currentTime.getTime() - 60 * 60 * 1000
      );
      currentTimeForDataCollection = new Date(
        startTimeForDataCollection.getTime() + this.timeOffset
      );
    ...
    if (
      this.noOfDivvyDataSamplesProcessed < this.noOfDivvyDataSamplesRequested
    ) {
      this.placesService
        .get_all_divvy_stations_data(this.timeRangeSelected, timeRangeSelection)
        .subscribe((data: Station[]) => {
          //console.log('getDivvyStationsStatus -- noOfDivvyDataSamplesProcessed=', this.noOfDivvyDataSamplesProcessed);
          this.noOfDivvyDataSamplesProcessed =
            this.noOfDivvyDataSamplesProcessed + 1;
          ...
          if (this.timeRangeSelected == this.PAST_HOUR) {
            this.timeOffset =
              this.noOfDivvyDataSamplesProcessed * (60 * 1000 * 2);
          } else if (this.timeRangeSelected == this.PAST_24_HOURS) {
            this.timeOffset =
              this.noOfDivvyDataSamplesProcessed * (60 * 60 * 1000);
          } else if (this.timeRangeSelected == this.PAST_7_DAYS) {
            this.timeOffset =
              this.noOfDivvyDataSamplesProcessed * (24 * 60 * 60 * 1000);
          }
          ...
          let monthNumber = this.checkForSingleDigitAndPrefixZeroIfNeeded(
            currentTimeForDataCollection.getMonth() + 1
          );
          simulatedClockTime =
            currentTimeForDataCollection.getFullYear() +
            "-" +
            monthNumber +
            "-" +
            this.checkForSingleDigitAndPrefixZeroIfNeeded(
              currentTimeForDataCollection.getDate()
            ) +
            "\t" +
            this.checkForSingleDigitAndPrefixZeroIfNeeded(
              currentTimeForDataCollection.getHours()
            ) +
            ":" +
            this.checkForSingleDigitAndPrefixZeroIfNeeded(
              currentTimeForDataCollection.getMinutes()
            ) +
            ":" +
            this.checkForSingleDigitAndPrefixZeroIfNeeded(
              currentTimeForDataCollection.getSeconds()
            );
          ...
          this.timer = setTimeout(
            () => this.getDivvyStationsStatus(this.notNewTimeRangeSelection),
            300
          );
        });
    }
  ...
  plot_availableDocksInDivvyStations_on_heatMap(availableDocksInDivvyStations) {
    ...
    for (let i = 0; i < this.stations.length; i++) {
      ...
      locationsOfAvailableDocksInDivvyStations.push(
        divvy_dock_station_location
      );
    }
