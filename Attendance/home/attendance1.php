...
$sql3 = "SELECT * from t2 WHERE roll=".$roll;
...
$percentage=($present*100)/$total;
	...
	$sql2 = "UPDATE t2 SET present='.$present.' ,total='.$total.',percentage='.$percentage.' WHERE roll=".$roll;

if ($conn->query($sql2) === TRUE) {
} else {
    echo "Error updating record: " . $conn->error;
}
$sql4 = "UPDATE t6 SET percentage='.$percentage.' WHERE roll=".$roll;
...
$percentage=($present*100)/$total;}
	...
	$sql2 = "UPDATE t2 SET absent='.$absent.' ,total='.$total.',percentage='.$percentage.' WHERE roll=".$roll;
